...
動画WFは`MiniMaxH3HybridLoader`でFL2VAをベースにRef2VAのAdaLN block 30–49を重ね、その出力にTurbo LoRAを適用する。モデル、カスタムノードの導入手順、取得元及びSHA-256は[導入マニュアル](../docs/installation.md#動画workflowの既定h3-hybrid-loader)を参照する。

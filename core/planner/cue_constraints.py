...
def build_grounded_cue_grammar(slots: Sequence[Mapping[str, object]]) -> str:
    ...
    rows = []
    tail = ('"｜接触=" ("禁止" | "許可") "｜現象=" '
            '("なし" | "外部自律" | "身体操作") "｜配置=対象位置:" cell '
            '"；人物位置:" cell '
            '"｜可視展開=" cell "｜身体主導=" cell "｜終端=" cell')
    none = quote("なし｜根拠=なし｜感情=") + ' cell ' + quote("｜接触=禁止｜現象=なし｜配置=なし｜可視展開=なし｜身体主導=") + ' cell "｜終端=" cell'
    for slot in slots:
        number = slot["slot"]
        selected_staging = slot.get("selected_staging_candidate")
        staging_anchor = (
            str(selected_staging.get("anchor", "")).strip()
            if isinstance(selected_staging, Mapping) else ""
        )
        slot_tail = (
            tail.replace('"｜配置=対象位置:" cell', quote("｜配置=対象位置:" + staging_anchor))
            if staging_anchor and staging_anchor != "なし" else tail
        )
        ...
        alternatives = [] if candidates or (staging_anchor and staging_anchor != "なし") else [none]
        for index, source in enumerate(sources):
            ...
            if candidates is not None:
                for candidate in candidates:
                    ...
                    if candidate["evidence"] == source and target and target in source:
                        ...
                        anchored_tail = (
                            slot_tail if staging_anchor and staging_anchor != "なし"
                            else tail.replace('"｜配置=対象位置:" cell', quote("｜配置=対象位置:" + target) + ' cell')
                        )
                        if candidate["kind"] == "effect":
                            # The discovery model chose an effect, not its origin.
                            # Preserve that target while leaving autonomous and
                            # hand-origin staging to the Scene interpretation.
            ...
            alternatives.append(f"{prefix}-target " + quote("｜根拠=" + source + "｜感情=") + " cell " + slot_tail)
        if not alternatives:

...
未指定の項目だけを補います。`# 演出候補`は出来事・人物演技・Camera担当へ直接渡す
任意の着想です。Camera担当は現在Sceneの歌詞と確定演技に合う撮影意図だけを
選択的に使い、候補から新しい対象や人物動作を追加しません。共通H3 promptや
完成EMDへ候補一覧を配布せず、作者固定Cameraを優先します。
短い歌唱句を顔いっぱいの接写で見せる候補も記述できます。この場合は両眉・両目・鼻・
口全体を収め、肩・手は必須にしません。身体演技や接触を見せる時間は必要な可視範囲を
維持し、歌唱句の境界で身体演技へ戻します。候補の採用やH3での構図・同期は保証しません。

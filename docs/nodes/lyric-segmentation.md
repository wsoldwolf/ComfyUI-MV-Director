...
Whisperが一部の短窓だけで文字列を返しながらword timestampを返さない場合、その窓だけを警告付きで除外し、ほかの短窓で得たtimestamp付き音響証拠を保持します。全短窓にtimestampがない場合は推測で補完せず、未配置歌詞として停止します。

長い未解決範囲から一部だけを回収できた場合は、回収済み区間を新しい前後アンカーとして残りを狭い範囲で再探索します。各passで新しい実timestampが増えた間だけ反復し、進展がなければ停止します。

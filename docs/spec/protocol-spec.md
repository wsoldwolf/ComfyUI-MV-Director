...
外部effectは自律的な出現を選べるが、Sceneの歌詞とユーザーの演出候補に合う場合は人物の手元を起点にしてもよい。どちらも対象自身の可視変化を必要とし、背景inventoryだけから新しい対象を活性化しない。
...
Planner v79以降、bounded Visual Beatの九field内の`配置`は
`対象位置:...；人物位置:...`の二節をGBNFで要求する。対象位置は歌詞で認可された
対象名から始まり、その支持物・場所を表す。人物位置は人物自身の足場と対象との
距離を表す。演出候補が選ばれた場合、LLMによるanchorは候補本文にも現在Sceneの
歌詞にも同じ対象名がある場合だけ採用し、`対象位置`へ完全一致で転送する。
身体演技だけの候補はVisual Beatの配置入力へ渡さず、Scene SpineとActionへ渡す。

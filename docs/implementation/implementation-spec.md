...
実装状態（2026-09-16）: Phase 0とPhase 1はFake backendによる純Pythonテストまで完了。Phase 2もGGUF探索、runtime設定、lazy llama.cpp lifecycle、serialized prompt token計測、stream中断、context予算、cache key及び成功専用file cacheをFake backendで実装済み。Phase 3はVision行parser、Subject EMD renderer、Context Loop adapter及び`auto_h3` graph bindingまで実装し、実Vision model adapterとComfyUI wrapperは未実装。

...
青はLLMへ渡す処理、緑はPythonが所有する決定的処理、橙は利用者又は上流からの入力です。Direction Enhancerは全体方針を一度だけ統合します。Timeline Planner v64は六つの中核taskへ処理を分け、`anime_emotional_mv`では条件付きのLyric Cue DiscoveryとScene spineを挟みます。

## Direction Enhancer

Direction Enhancerは`retention_policy`、人物`concept_emd`、背景`scene_emd`、Style/Motion/Camera profileに加え、任意の自由文による追加プロンプトを`user_request`から受け取ります。この追加プロンプトは全Scene共通であり、個別Sceneへの局所指示ではありません。外部profileの構造、固定section及びauthorityはPythonが検証し、LLMは固定されていない`STYLE`、`ENVIRONMENT`、`TIME_LIGHTING`、`MOTION`、`CAMERA`、`OTHER`行だけを返します。欠落slotは局所retryし、Pythonが`MVD_DIRECTION_V3`と人間向けpreviewを構築します。
...
| Scene spine | 条件付きLLM + Python | 有効Cueを持つ複数Shotの`dance_phrase` Sceneだけ、全Shotの準備→一回のevent→反応、対象のcoverageを一括計画。不正なら局所retry後に従来経路へ戻す |
